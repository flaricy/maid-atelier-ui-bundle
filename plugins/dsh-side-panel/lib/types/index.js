...
async function preview(root, input, maxText, maxImage) {
    ...
    if (info.size === 0)
        return { kind: 'empty', path: target.path, name, size: 0 };
    const mime = IMAGE_MIME[extension];
    if (mime) {
        if (info.size > maxImage)
            return { kind: 'too-large', path: target.path, name, size: info.size };
    ...
    const body = await readFile(target.absolute);
    if (body.includes(0))
        return { kind: 'binary', path: target.path, name, size: info.size };
...
export function apply(ctx, config = {}) {
    ...
    ctx.on('session/event', ((session, event) => {
        ...
        state.ready = state.ready.then(async () => {
            ...
            else if (state.before !== undefined) {
        ...
        const disposeRoute = ctx.webServer.register({
            ...
            handler: async (req, res) => {
                try {
                    ...
                    if (action === 'search')
                        return json(res, 200, { ok: true, ...await search(root, path, searchMaxResults) });
                    if (action === 'preview')
                        return json(res, 200, { ok: true, preview: await preview(root, path, maxText, maxImage) });
                    if (action === 'write') {
                    ...
                    if (action === 'review') {
                        const allowedModes = new Set(['unstaged', 'staged', 'commits', 'branches', 'last-session']);
                        ...
                        if (untrackedDiffs.length > 0)
                            diff += `${diff.endsWith('\n') || diff === '' ? '' : '\n'}${untrackedDiffs.join('\n')}`;
                        const commits = mode === 'commits' ? await execFileAsync('git', ['log', '-50', '--date=relative', '--pretty=format:%H%x1f%h%x1f%s%x1f%an%x1f%ar%x1f%D%x1e'], { cwd: root, maxBuffer: 1024 * 1024 }).then(value => value.stdout.split('\x1e').filter(Boolean).map(record => {
                        ...
                        const branches = mode === 'branches' ? await execFileAsync('git', ['for-each-ref', '--format=%(refname:short)%00%(HEAD)%00%(upstream:short)%00%(upstream:track,nobracket)%00%(subject)', 'refs/heads'], { cwd: root, maxBuffer: 1024 * 1024 }).then(value => value.stdout.split('\n').filter(Boolean).map(line => {
                    ...
                    if (action === 'terminal-open') {
                        ...
                        const record = { owner: sessionId, process: terminal, chunks: [], exited: false };
                    ...
                    if (action.startsWith('terminal-')) {
                        ...
                        if (record === undefined || record.owner !== sessionId)
