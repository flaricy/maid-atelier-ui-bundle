...
import { createMaidAtelierTitlebarBrand } from './titlebar-brand.ts'
...
/** Place the skin's neutral identity in the frameless title bar. */
function decorateTitlebarBrand(ownedNodes: Set<Element>): void {
  ...
  brand.append(createMaidAtelierTitlebarBrand())
...
export function apply(ctx: Context): void {
  ...
  const syncBackdrop = (): void => {
    ...
    const source = dark ? MAID_ATELIER_PALACE_DARK : MAID_ATELIER_PALACE_LIGHT
    const veil = dark
      ? 'linear-gradient(rgba(5, 12, 30, 0.52), rgba(5, 12, 30, 0.52))'
      : 'linear-gradient(rgba(78, 96, 139, 0.22), rgba(78, 96, 139, 0.22))'
    body.style.setProperty('background-image', `${veil}, url(${source})`)
  }
  syncBackdrop()
