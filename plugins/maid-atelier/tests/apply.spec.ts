...
describe('Maid Atelier skin apply', () => {
  ...
  it('installs an inlined background and restores prior body styles', async () => {
    ...
    expect(document.body.style.backgroundImage).toContain('linear-gradient(rgba(78, 96, 139, 0.22)')
  ...
  it('masks transcript copy beneath the sticky composer without flattening the canvas', () => {
    expect(CSS).toMatch(
      /\[data-phase='active'\] \[data-composer-seat\]\s*\{[^}]*rgba\(220, 230, 245, 0\.97\) 36px/s,
    )
    expect(CSS).toMatch(
      /\[data-ds-dark-theme\][\s\S]*?\[data-phase='active'\] \[data-composer-seat\]\s*\{[^}]*rgba\(8, 15, 39, 0\.97\) 36px/s,
    )
  ...
  it('keeps the inline subagent catalog readable inside the dark header', () => {
    expect(CSS).toMatch(
      /button\[aria-haspopup='tree'\] \+ \[role='tree'\]\s*\{[^}]*--dsw-alias-label-primary: #1b294a[^}]*background: #f7f9fd[^}]*text-shadow: none/s,
    )
    expect(CSS).toMatch(
      /button\[aria-haspopup='tree'\] \+ \[role='tree'\][\s\S]*?:is\(\[class\*='summary'\], \[class\*='metrics'\]\)\s*\{[^}]*color: #596989[^}]*font-size: 12px/s,
    )
    expect(CSS).toMatch(
      /\[data-ds-dark-theme\][\s\S]*?button\[aria-haspopup='tree'\] \+ \[role='tree'\]\s*\{[^}]*--dsw-alias-label-primary: #edf1fa[^}]*background: #142044/s,
    )
    expect(CSS).not.toMatch(
      /:is\(\[role='dialog'\], \[role='menu'\], \[role='tree'\], \[data-radix-popper-content-wrapper\]/,
    )
  })
  ...
  it('places an original text-only lockup in the frameless title bar', async () => {
    fiber = await mount()
    ...
    expect(brand?.querySelector('svg, path, img')).toBeNull()
    expect(brand?.querySelector('[data-maid-titlebar-monogram]')?.textContent).toBe('DSH')
    expect(brand?.querySelector('[data-maid-titlebar-name]')?.textContent).toBe('MAID ATELIER')
    expect(brand?.querySelector('[data-maid-titlebar-separator]')).not.toBeNull()
    await fiber.dispose()
    expect(document.querySelector("[data-skin-chrome='titlebar-brand']")).toBeNull()
  })

  it('styles the title-bar lockup centered on the window, always visible', () => {
    ...
    expect(CSS).toMatch(/\[data-maid-titlebar-lockup\]\s*\{[^}]*height: 18px/s)
    expect(CSS).toMatch(/\[data-maid-titlebar-monogram\]\s*\{[^}]*font-size: 10px/s)
    expect(CSS).toMatch(/\[data-maid-titlebar-separator\]\s*\{[^}]*transform: rotate\(45deg\)/s)
    // The lockup must not hide with the rail: it is decorative and centered.
  ...
  it('switches between matched day and night backgrounds with the base theme', async () => {
    ...
    expect(dark).toContain('linear-gradient(rgba(5, 12, 30, 0.52)')
